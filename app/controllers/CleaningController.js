...
const ImageDAO = require("../database/DAO/ImageDao");
...
const CleaningController = {
    ...
    async insertImageCleaning(req, res) {
        try{
            if(req.body != undefined && req.body != []){
                if(req.file != undefined){
                    if(Utils.notEmpty(productModel.id)){
                        let imageModel = new ImagesModel({id_user: req.body.id_user, link: 'uploads/'+req.file.filename, profile: req.body.profile, after: req.body.after, id_cleaning});
                        let intBD = await ImageDAO.insertImageCleaning(imageModel);
                        if(intBD.result){
                            res.status(200).json({
                                'type': "S",                            
                                'message': 'Sucesso ao cadastrar imagem da faxina',
                            });
                        }else{
                            res.status(500).json({
                                'type': "E",                            
                                'message': 'Erro ao cadastrar imagem da faxina'
                            });
                        }
                    }else{
                        res.status(400).json({ 
    ...
    async deleteImageCleaning(req, res) {
        try{
            if(req.body != undefined && req.body != []){
                ...
                if(req.body.imagesDelete != null && req.body.imagesDelete != undefined ){
                    ...
                    if(imagesDeleteId.length > 0){
                        for(var i = 0; i < imagesDeleteId.length; i++){
                            ...
                            await ImageDAO.deleteImageCleaning(imagesDeleteId[i]); 
                        }
